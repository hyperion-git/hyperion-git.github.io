...
- **Affiliation is dated.** Until **2026-12-01** the site shows no affiliation at all: the subtitle is just "Physicist", no `more_info`, no ClockWerQ or DLR — even if asked to "complete" the bio. From **2026-12-01** add ClockWerQ GmbH (CTO) to the `_pages/about.md` subtitle / `more_info` and to `description` in `_config.yml`. Never DLR. This tracks the paper-affiliation switch in `~/dev/papers/CLAUDE.md` § Author block.
...
- **Deploy** (live since 2026-09-21): push to `main` → `Deploy site` workflow (production build, purgecss, JamesIves) → `gh-pages` → GitHub Pages in legacy mode with source `gh-pages`/`/`. Check runs with `gh run list -R hyperion-git/hyperion-git.github.io` (a bare `gh run list` picks the `upstream` remote). Ordinary pushes trigger it (verified 2026-09-21); only the initial push, which carried the workflow file itself, did not, and `gh workflow run "Deploy site" -R hyperion-git/hyperion-git.github.io --ref main` dispatched it by hand.
- **Layout overrides** (tracked drift points, `bundle exec al-folio upgrade overrides audit`): `_layouts/about.liquid`, `_layouts/page.liquid`, `_layouts/bib.liquid` (badge links to DOI/arXiv), plus the site-local `_layouts/note.liquid` and `_includes/site_profile.liquid` (round picture with the social icons beneath it top-right on every page: `page.profile_picture` when set, else the portrait `profile_image` from `_config.yml`; the location appears on the about page only; the four section pages carry their own 800×800 JPEG in `assets/img/` (`publications.jpg`, `projects.jpg`, `notes.jpg`, `blog.jpg`; master size 800 px because the responsive-image plugin never upscales, so larger masters only add repo weight; `publications.jpg` and `notes.jpg` are crops of the owner's originals chosen so that the disc shows board only / the whole sketch on neutralised paper), `placeholder.svg` stays as the round stand-in for any new page; on phones the block sits centred above the heading).
- **Design**: day = white ground, CWQ navy ink `#162D3D`, AFP blue `#2E2CB8` accent; night = the figstyle `cwq` on-dark set (ground `#0F212D`, panels `#1E3B4E`, ink `#E7EDF1`) with the CWQ H-maser blue lifted to `#7EA4D1` as accent (owner's choice 2026-09-21; navy is the constant) and the ClockWerQ type system (Source Sans 3 Light/Semibold, Source Code Pro), self-hosted woff2 in `assets/fonts/` (SIL OFL). All tokens and type rules live in `_sass/_site.scss`, pulled in by the shadowed `assets/css/main.scss` (a copy of the gem's entry plus `@use "site"`). Colours are set as CSS custom properties there — a shadowed `_sass/_variables.scss` does **not** reach gem partials. The font `<link>` slot `third_party_libraries.google_fonts.url.fonts` points at `/assets/css/fonts.css`; keep it self-hosted (owner's decision, GDPR).
- **No third-party requests** (owner's decision, GDPR): `third_party_libraries.download: true` self-hosts every library into `assets/libs/` at build time (git-ignored except the vendored icon webfonts in `assets/libs/webfonts/` and `assets/libs/fonts/`, which the plugin does not fetch and which must match `fontawesome.version` / `academicons.version`). MathJax's `integrity` pin is intentionally blank (the local copy is re-minified). `enable_medium_zoom`, Altmetric and Dimensions badges are off for the same reason. Keep `_layouts/bib.liquid` out of Prettier (its two-line front matter gets folded).
- **Licence**: MIT for the whole site (LICENSE carries both the al-folio and the owner's copyright line); the copyright statement lives in the Impressum, not in the footer.
- **Impressum**: `_pages/impressum.md` (footer link via `impressum_path`) carries the owner's postal address and the contact address `contact@randomfluctuations.net`, emitted only through `{% al_email_protect_link %}` (`protect_email: true`); never write that address in plain text anywhere on the site.
