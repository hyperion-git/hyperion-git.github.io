...
- **Design**: day = white ground, CWQ navy ink `#162D3D`, AFP blue `#2E2CB8` accent; night = the figstyle `cwq` on-dark set (ground `#0F212D`, panels `#1E3B4E`, ink `#E7EDF1`) with the CWQ note blue lifted to `#66ACDE` as accent (owner's choice 2026-09-21; navy is the constant) and the ClockWerQ type system (Source Sans 3 Light/Semibold, Source Code Pro), self-hosted woff2 in `assets/fonts/` (SIL OFL). All tokens and type rules live in `_sass/_site.scss`, pulled in by the shadowed `assets/css/main.scss` (a copy of the gem's entry plus `@use "site"`). Colours are set as CSS custom properties there — a shadowed `_sass/_variables.scss` does **not** reach gem partials. The font `<link>` slot `third_party_libraries.google_fonts.url.fonts` points at `/assets/css/fonts.css`; keep it self-hosted (owner's decision, GDPR).
