...
- **Design**: AFP palette and the ClockWerQ type system (Source Sans 3 Light/Semibold, Source Code Pro), self-hosted woff2 in `assets/fonts/` (SIL OFL). All tokens and type rules live in `_sass/_afp.scss`, pulled in by the shadowed `assets/css/main.scss` (a copy of the gem's entry plus `@use "afp"`). Colours are set as CSS custom properties there — a shadowed `_sass/_variables.scss` does **not** reach gem partials. The font `<link>` slot `third_party_libraries.google_fonts.url.fonts` points at `/assets/css/fonts.css`; keep it self-hosted (owner's decision, GDPR).
