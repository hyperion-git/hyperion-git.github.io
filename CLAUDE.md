...
- **Layout overrides** (tracked drift points, `bundle exec al-folio upgrade overrides audit`): `_layouts/about.liquid`, `_layouts/page.liquid`, `_layouts/bib.liquid` (badge links to DOI/arXiv), plus the site-local `_layouts/note.liquid` and `_includes/site_profile.liquid` (portrait + icons top-right on every page: `page.profile_picture` when set, else `profile_image` from `_config.yml`; the four section pages carry their own 800×800 JPEG in `assets/img/` (`publications.jpg`, `projects.jpg`, `notes.jpg`, `blog.jpg`; master size 800 px because the responsive-image plugin never upscales, so larger masters only add repo weight), `placeholder.svg` stays as the round stand-in for any new page; on phones the block sits centred above the heading).
