...
- **Deploy** (live since 2026-09-21): push to `main` → `Deploy site` workflow (production build, purgecss, JamesIves) → `gh-pages` → GitHub Pages in legacy mode with source `gh-pages`/`/`. Check runs with `gh run list -R hyperion-git/hyperion-git.github.io` (a bare `gh run list` picks the `upstream` remote). The very first push did not trigger the workflow; `gh workflow run "Deploy site" -R hyperion-git/hyperion-git.github.io --ref main` dispatches it by hand.
